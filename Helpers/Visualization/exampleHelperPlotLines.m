function lineHandle = exampleHelperPlotLines(varargin)%(s1,s2,s3,s4,s5)
%exampleHelperPlotLines Display individual or pair-wise connected N-by-[x y] lines
...
% Copyright 2023 The MathWorks, Inc.
    ...
    for i = 1:2:numel(lineSpecs)
        ...
        if ~isempty(lineSpecs{i})
            [x,y] = getXYSingle(p2);
            lineHandle(end+1,1) = plot(x(:),y(:),lineSpecs{i}{:});
            hold on;
        end
        if ~isempty(lineSpecs{i+1})
            ...
            if isempty(p1)
                ...
                [x,y] = mergeXY(p1,p2);
                lineHandle(end+1,1) = plot(x(:),y(:),lineSpecs{i+1}{:});
            end
        end
        grpIdx = grpIdx+1;
    end
...
function [x,y] = mergeXY(p1,p2)
%mergeXY Create line-segments between two lines
    sz1 = size(p1,1);
    sz2 = size(p2,1);
    if size(p1,2) == 1
...
function [x, y] = getXYSingle(s1)
%getXYSingle Plot 2D line that has been provided as column vector
    if size(s1,2) == 1
        x = 1:size(s1,1);
        y = s1(:,1);
    else
...
function spec = spec2cell(specIn)
%spec2cell Wrap inputs in cell-array for uniformity
    if ~iscell(specIn)
        spec = {specIn};
    else
        spec = specIn;
